...
export default function CompanyDeliverySelector({
  ...
  if (isLoading) {
    return (
      <div className="p-4 bg-gray-100 rounded-lg text-center" role="status">
  ...
  if (data.length === 0) {
    return (
      <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-center" role="alert">
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4" role="radiogroup" aria-label="Compañías de delivery disponibles">
        {data.map((company) => (
          <CompanyOption
            key={company.id}
            company={company}
  ...
  return (
    <button
      ...
      className={`
        ...
        text-left relative flex flex-col gap-2
        ${
          isSelected
